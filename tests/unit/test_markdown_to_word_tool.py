...
"""
        tool_parameters = {
            'markdown_text': markdown_text,
            'output_file': 'test_ordered_list.docx'
        }
        ...
        if self.generated_filename:
            ...
            self.assertTrue(output_path.exists(), f"Word文档应该已保存到: {output_path}")
            self.assertGreater(os.path.getsize(output_path), 0, "生成的Word文档不应为空")
            print(f"\n✅ 有序列表测试文档已保存到: {output_path.absolute()}")
    
    def test_nested_list_indent(self):
        """测试嵌套列表的缩进"""
        markdown_text = """# 测试嵌套列表缩进
...
这是另一个段落，用于验证列表缩进是否正确。
"""
        tool_parameters = {
            'markdown_text': markdown_text,
            'output_file': 'test_nested_list_indent.docx'
        }
        ...
        if self.generated_filename:
            ...
            self.assertTrue(output_path.exists(), f"Word文档应该已保存到: {output_path}")
            self.assertGreater(os.path.getsize(output_path), 0, "生成的Word文档不应为空")
            print(f"\n✅ 嵌套列表缩进测试文档已保存到: {output_path.absolute()}")
